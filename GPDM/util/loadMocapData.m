...
globY = Y(2:end, 1:3) - Y(1:end-1, 1:3); % 保留头三个维度的数据，后一帧的数据减去前一帧的数据
globY(end_segments,1:3) = globY(end_segments-1,1:3); % 最后一帧的倒数第二帧
Y = [globY Y(:, 4:end)]; % 把第4到最后一个维度的数据拼在globY数据之后

varY = zeros(1,2);
numJoints = (size(Y,2) - 3);  % 这个数据没有用
...
for n = 1:N
    if (endFrames(n) > 0)
        curY{n} = curY{n}(startFrames(n):skipFrames(n):endFrames(n), :);  % 保留其中130帧数据
    else
...
globY = Y(2:end, 1:3) - Y(1:end-1, 1:3); % 头三维数据，后一帧减去前一帧的数据
globY(end_segments,1:3) = globY(end_segments-1,1:3); % 最后一帧的数据等于倒数第二帧的数据
Y = [globY Y(:, 4:end)]; %剩下维度的数据拼接到头三维数据后
